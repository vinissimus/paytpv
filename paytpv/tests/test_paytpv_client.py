...
async def test_add_user_async(paytpv_async, event_loop):
    ...
    # remove added users
    res = await paytpv_async.remove_user(
        idpayuser=DS_IDUSER, tokenpayuser=DS_TOKEN_USER
    )
    res.DS_RESPONSE == 1
    res.DS_ERROR_ID == 0
    res = await paytpv_async.remove_user(
        idpayuser=DS_IDUSER_2, tokenpayuser=DS_TOKEN_USER_2
    )
    res.DS_RESPONSE == 1
    res.DS_ERROR_ID == 0
...
def test_info_user(paytpv, user):
    ...
    assert (
        res.DS_CARD_HASH
        == "d752d8a349d88ba10f5d09f2ec09baba7b527d82d3fdaef175048a15e19e34bc"
    )
...
def test_execute_purchase(paytpv, user):
    ...
    with pytest.raises(PaytpvException) as e:
        res = paytpv.execute_purchase(
            idpayuser="0", tokenpayuser="0", amount=33, order="3"
        )
